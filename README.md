...
Most recently, the WZDx Working Group has evolved into the Work Zone Data Working Group (WZDWG), established under the Federal Geographic Data Committee (FGDC) Transportation Subcommittee (TSC) to refine the features, attributes, and vocabulary needed to model work zone activity data. Currently, the WZDWG’s priority is to update WZDx v1.1, developing version 2.0 of the specification for increased adoption by end-users. More information is available on the [Wiki](https://github.com/usdot-jpo-ode/jpo-wzdx/wiki).
