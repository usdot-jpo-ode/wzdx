# WZDx Example Feeds
This directory contains GeoJSON documents that are example outputs from valid WZDx data feeds. The examples cover a range of common scenarios. There are examples for both the `WZDxFeed` and `SwzDeviceFeed`, contained within the respective subdirectories.
