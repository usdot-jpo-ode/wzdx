...
[Scenario 4](/create-feed/examples/linestring-examples/scenario4_detour_linestring_example.geojson) is based on the same numbered annotated work zones in [Discussion Topic 131](https://github.com/usdot-jpo-ode/jpo-wzdx/discussions/131) which has an associated detour for a road closure.  The example is slightly modified for use on a multi-lane facility.
...
The [local access only example](/create-feed/examples/linestring-examples/local_access_only_bidirectional_linestring_example.geojson) shows a basic work zone that keeps all lanes open but places restrictions to only local traffic.
...
The [bridge height restriction example](/create-feed/examples/linestring-examples/bridge_restriction_linestring_example.geojson) shows how the restriction road event type can be used within a feed.
