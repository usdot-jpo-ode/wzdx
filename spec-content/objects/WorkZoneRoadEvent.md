...
`is_start_position_gps_verified` | Boolean | Indicates if the start position (first geometric coordinate pair) is based on actual reported data from a GPS-equipped device that measured the location of the start of the work zone. | Conditional; required if `beginning_accuracy` is not provided. |
`is_end_position_gps_verified` | Boolean | Indicates if the end position (last geometric coordinate pair) is based on actual reported data from a GPS-equipped device that measured the location of the end of the work zone. | Conditional; required if `ending_accuracy` is not provided. |
...
`beginning_accuracy` (DEPRECATED) | [SpatialVerification](/spec-content/enumerated-types/SpatialVerification.md) | *This property is deprecated and will be removed in a future version, use `is_start_position_gps_verified` instead.* Indicates how the beginning coordinate was defined. | Conditional; required if `is_start_position_gps_verified` is not provided. |
`ending_accuracy` (DEPRECATED) | [SpatialVerification](/spec-content/enumerated-types/SpatialVerification.md) | *This property is deprecated and will be removed in a future version, use `is_end_position_gps_verified` instead.* Indicates how the ending coordinate was defined. | Conditional; required if `is_end_position_gps_verified` is not provided. |
