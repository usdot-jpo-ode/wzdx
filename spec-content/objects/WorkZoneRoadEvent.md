...
`start_date` | String; [date-time](https://tools.ietf.org/html/draft-handrews-json-schema-validation-01#section-7.3.1) | The UTC time and date when the event begins. **Business Rule**: All datetime formats shall follow [RFC 3339 Section 5.6](https://tools.ietf.org/html/rfc3339#section-5.6). | Required  | `2016-11-03T19:37:00Z` |
`end_date` | String; [date-time](https://tools.ietf.org/html/draft-handrews-json-schema-validation-01#section-7.3.1)  | The UTC time and date when the event ends. **Business Rule**: All datetime formats shall follow [RFC 3339 Section 5.6](https://tools.ietf.org/html/rfc3339#section-5.6). | Required  | `2016-11-03T19:37:00Z` |
...
`beginning_milepost` | Number | The linear distance measured against a milepost marker along a roadway where the event begins. **Business Rule**: Include only as many significant figures as your equipment can accurately measure. | Optional | 10.50 |
`ending_milepost` | Number | The linear distance measured against a milepost marker along a roadway where the event ends. **Business Rule**: Include only as many significant figures as your equipment can accurately measure. | Optional | 15.85 |
...
`reduced_speed_limit_kph` | Number | The reduced speed limit posted within the road event, in kilometers per hour. This property only needs to be supplied if the speed limit within the road event is lower than the posted speed limit of the roadway. **Business Rule**: This property only needs to be supplied if the speed limit within the road event is lower than the posted speed limit of the roadway.| Optional | 45 |
`restrictions` | Array; [[Restriction](/spec-content/objects/Restriction.md)] | A list of zero or more road restrictions that apply to the roadway segment described by this road event. **Note**: Restrictions can also be provided on an individual lane.| Optional | {“type”:“reduced-height”, “value”:11.75, “unit”:“feet”} |
